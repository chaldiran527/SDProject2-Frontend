...
const handleBuscarAlfa = () => {
  ...
  fetch('/api/estudianteRoutes/listar_estudiantes')
    ...
    .then((data) => {
      ...
      // Ordenar los estudiantes filtrados alfabéticamente por nombre
      estudiantesFiltrados.sort((a, b) => a.nombre.localeCompare(b.nombre));
      // Actualizar el estado de consultas con los estudiantes filtrados
      setconsultas(estudiantesFiltrados);
      setExcelVisible(true); // Mostrar el botón de exportar a Excel
...
const handleDescargarExcel = () =>{
  const consultasSinFoto = consultas.map(({ foto, ...rest }) => rest);
  ...
  const ws = XLSX.utils.json_to_sheet(consultasSinFoto);
